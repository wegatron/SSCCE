  ...
  // , selectionFinished_( true )
{
  ...
  osg::ref_ptr<osg::Node> loadedModel = osgDB::readNodeFile("/home/wegatron/workspace/ui_framework/branches/4zsw/data/sapin_up/sapin_up.obj");
  ...
  viewer_->addEventHandler( new ui_handler );
  viewer_->setCameraManipulator( new osgGA::TrackballManipulator ); // very important
...
bool ui_handler::handle(const osgGA::GUIEventAdapter& ea,osgGA::GUIActionAdapter& aa)
{
  ...
  case(osgGA::GUIEventAdapter::PUSH): {
    ...
    int mouse_button_mask = ea.getButtonMask();
    if(mouse_button_mask == osgGA::GUIEventAdapter::MouseButtonMask::LEFT_MOUSE_BUTTON) {
      //view->setCameraManipulator(0);
      //std::cerr << " set camera manipulator to null" << std::endl;
