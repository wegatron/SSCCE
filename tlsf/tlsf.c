...
#define BLOCK_SIZE	(0xFFFFFFFF - PTR_MASK) /* minum block size is 16 bytes, use & BLOCK_SIZE to eliminate the tail bits */
...
static __inline__ void MAPPING_INSERT(size_t _r, int *_fl, int *_sl)
{
    if (_r < SMALL_BLOCK) {
        ...
        *_fl = ms_bit(_r);
        *_sl = (_r >> (*_fl - MAX_LOG2_SLI)) - MAX_SLI; // refer to tlsf mapping equation
        *_fl -= FLI_OFFSET; // FLI_OFFSET = \log_2 SMALL_BLOCK - 1
    }
}
